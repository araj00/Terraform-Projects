...
resource "aws_s3_bucket_policy" "allow_access_to_specific_vpce_only" {
  ...
  depends_on = [
    aws_vpc_endpoint.s3-vpc-endpoint,
    aws_s3_bucket_server_side_encryption_configuration.my-bucket-s3,
...
resource "aws_s3_bucket_public_access_block" "my_bucket_pab" {
  ...
  depends_on = [aws_s3_bucket_policy.allow_access_to_specific_vpce_only]
}